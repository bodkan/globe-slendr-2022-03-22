...
Let's take this minimalistic simulation script:
...
**The above simulates 2 $\times$ 10000 chromosomes, 100 Mb each.**
...
Simulate a tree sequence using `msprime()` from your model (perhaps 50Mb?), add mutations with `ts_mutate()`.
...
::: aside
*Hint*: $f_4$ (W = AFR, X = EUR; Y = NEAND, Z = CHIMP) is the test setup where the null hypothesis states that the value of this statistic is "consistent with zero". Negative value indicates gene flow NEAND-EUR.
:::
...
::: aside
Hint: You can compute Neanderthal ancestry for a vector of individual names `X` as `ts_f4ratio(ts, X = X, "NEA_1", "NEA_2", "AFR_1", "CHIMP_1")` ("direct f4-ratio" from [Petr _et al_., PNAS 2019](https://www.pnas.org/doi/full/10.1073/pnas.1814338116)).
:::
