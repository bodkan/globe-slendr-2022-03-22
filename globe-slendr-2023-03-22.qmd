...
<h2>Solve 80% of modeling for popgen with 20% effort</h2>
...
## Mutations added on branches _after_!
...
## Tree sequences are very efficient
...
```{r}
ts <-
  population("pop", time = 10000, N = 10000) %>%
...
**Simulates 2 $\times$ 10000 chromosomes 100 Mb each.**
...
The first argument is _always_ a `ts` object.
...
Simulate a tree sequence using `msprime()` from your model (maybe 50Mb), add mutations with `ts_mutate()`.

<hr>

Compute [diversity ($\pi$)](https://en.wikipedia.org/wiki/Nucleotide_diversity) in each population (`ts_diversity(ts, ...)`). Do your results fit $N_e$ in your model?

<hr>

Compute `ts_divergence(ts, ...)` (genetic divergence) or `ts_fst(ts, ...)` ([$F_{ST}$](https://en.wikipedia.org/wiki/Fixation_index)) between pairs of populations. Do the results match the order of splits in your model?

::: aside
_Hint:_ See [manual page](https://www.slendr.net/reference/index.html#tree-sequence-statistics) of each tree-sequence statistic function for notes on how to use it.
:::
...
Test for the evidence of Neanderthal ancestry in Europeans vs Africans using the $f_4$-statistic implemented `ts_f4()` (also known as ABBA/BABA test or $D$ statistic).
